...
void setWindowCutOnMaximize(int value) { window_cut_on_maximize = value; }
...
void setWindowCanBeShown(bool value) { window_can_be_shown = value; }
...
void forceChildRefresh() {
  if (flutter_child_window == nullptr) return;

  RECT rc;
  GetClientRect(flutter_window, &rc);
  int width = rc.right - rc.left;
  int height = rc.bottom - rc.top;
  SetWindowPos(flutter_child_window, 0, 0, 0, width + 1, height + 1,
               SWP_NOMOVE | SWP_NOACTIVATE);
  SetWindowPos(flutter_child_window, 0, 0, 0, width, height,
               SWP_NOMOVE | SWP_NOACTIVATE);
}
...
LRESULT handle_nccalcsize(HWND window, WPARAM wparam, LPARAM lparam) {
  ...
  if (isMaximized) {
    ...
    int topCut =
        (int)ceil(scaleFactor * window_cut_on_maximize) + scaleFactorInt + 1;
    ...
    // border (because WM_NCHHITTEST is only received inside client area). In
    // modern Windows (i.e. 10 or 11), this space actually looks transparent,
    // thus it feels like the resize border is outside client area but it is
    // actually not, instead the actual client area size is reduced. The thing
    // to note here is that the top margins are not changed (see behavior of
    // file explorer for example) & |HTTOP| is handled on the parent HWND.
...
LRESULT handle_child_nchittest(HWND window, WPARAM wparam, LPARAM lparam) {
  // We don't need |HTTRANSPARENT| at top in maximized window.
  RECT rect;
  ::GetWindowRect(window, &rect);
  if (rect.top <= 0) {
    return HTCLIENT;
  ...
  // Other left, right & bottom locations are |HTCLIENT|.
...
LRESULT CALLBACK child_window_proc(HWND window, UINT message, WPARAM wparam,
  ...
  switch (message) {
    ...
    case WM_NCHITTEST: {
      ...
      return handle_child_nchittest(window, wparam, lparam);
      break;
    }
  }
...
LRESULT CALLBACK main_window_proc(HWND window, UINT message, WPARAM wparam,
                                  LPARAM lparam, UINT_PTR subclssID,
                                  DWORD_PTR refData) {
  switch (message) {
    case WM_ERASEBKGND: {
      return 1;
    ...
    case WM_NCCREATE: {
      ...
      style = style | WS_CLIPCHILDREN | WS_EX_TRANSPARENT ^ WS_CAPTION;
    ...
    case WM_CREATE: {
      ...
      centerOnMonitorContainingMouse(window, createStruct->cx,
                                     createStruct->cy);
    ...
    case WM_WINDOWPOSCHANGING: {
      ...
      if ((isShowWindow == TRUE) && (window_can_be_shown == FALSE) &&
